...
102. Funktionen hˆherer Ordnung
103. Currying 
...
## Flieﬂkommazahlen [Programmieren Lernen - Lerne alle Sprachen in einer Serie]
...
Um sich zuf‰llige Werte ausrechnen lassen zu kˆnnen, gibt es Zufallgeneratoren, die sich mit einer `Random`-Bibliothek in den Code einbauen lassen.  
`int i = random.random_int(1, 6);`  
Hier wird ien zuf‰lliger gannzahliger Wert zwischen 1 und 6 zur¸ckgegeben.  

Die meisten Programmiersprachen haben aber nur Funktionalit‰ten um einen Flieﬂkommazahlwert von 0 bis 1 errechnen zu kˆnnen.  
`float f = random.next();`  

Um hiermit nun das selbe Ergebnis mit einer Range von 1-6 erzielen zu kˆnnen, multipliziert man einfach diesen Wert mit 6.  
...
Um eine Datei verwenden zu kˆnnen, nutzt man eine `File`-Bibliothek.  
...
Um die Datei hinterher wieder verwenden zu kˆnnen, muss diese geschlossen werden `close()` und mit der `read()`-Methode kann man eine Anzahl von Zeichen ¸bergeben, die man lesen mˆchte.
...
Um den Cursor in der Datei verschieben zu kˆnnen, gibt es die `seek()`-Methode der man einen Wert ¸bergeben kann um an die Stelle in der Datei zu springen an der man mit den Cursor springen will.  
## Ascii [Programmieren Lernen - Lerne alle Sprachen in einer Serie]
[Ascii](https://de.wikipedia.org/wiki/American_Standard_Code_for_Information_Interchange) (American Standard for Information Interchange) enth‰lt keine Umlaute wie z.B. ‰,ˆ,¸, ist aber super f¸r die englische Sprache, enh‰lt 128 Zeichen und wir meiﬂtens in einem Byte gespeichert.  

### Unicode [Programmieren Lernen - Lerne alle Sprachen in einer Serie]
[Unicode](https://unicode-table.com/de/) ist ein neuerer Zeichensatz der viel mehr Zeichen (ca. 65.000) enth‰lt und damit auch einen hˆhrenen Speicherbedarf hat.  

Um diesen Zeichensatz in einem String verwenden zu kˆnnen nutz man `\u` + die Hexadezimalzahl des Zeichens welches man verwenden will.  
## Bin‰re Dateien [Programmieren Lernen - Lerne alle Sprachen in einer Serie]
Bin‰re Dateien beinhalten Zeichen die innerhalb von einem Byte (0-255) abgebildet werden kˆnnen.  

Bestes Beispiel hierf¸r sind Bild-Dateien, hier muss man gerade bei JPG-Dateien darauf achten einen JPG-Reader zu verwenden, da dieses Format im Gegensatz zu PNG-Dateien intern komprimiert ist.  
...
Bei der Serialisierung eines Objekts speichert man die Instanz einer Klasse in einer Datei, um diese Datei woanders wieder einlesen zu kˆnnen.  
...
XML wird nicht mehr oft benutzt, daf¸r aber JSON.  
`import json`  
`{`
    `"meineInstanz":`  
        `["A":{`  
...
Flushing wird benˆtigt um den Buffer bspw. bei Dateien freizugeben.  
`File f = ...`  
`int i = 0`  
`while (true){`  
    `f.write('a')`  
    ...
    `}`  
`}`  
 Hier wird immer bei `i = 1000` der gef¸llte Buffer in die Datei geschrieben.  
...
Der Absolute Pfad geht von dem Datentr‰ger aus auf dem sich die Datei befindet.  
`import File`  
...
Es gibt folgende Stages die man angeben kann, die dar¸ber entscheiden wie viele und welche Informationen man weiter gibt.  
NOTSET = Quasi keine Infromationen
DEBUG = nur f¸r den Programmierer
INFO = Informationen auf f¸r den Benutzer
...
Um mit Datum und Uhrzeit in einem Programm rechenen zu kˆnnen, benˆtigt man die `DateTime`-Bibliothek.  
`import DateTime`  
...
Um mit seinem Programm ¸ber das Internet kommunizieren zu kˆnnen, nutzt man beim Networking Socketserver um eine Interneverbindung herstellen zu kˆnnen. Dieses Konstrukt beinhaltet eine Server und eine CLientseite.  

Bei der Serverseite definiert man einen Port der mˆglichst nicht von anderen Programmen verwendet wird und wartet so lange bis von der Clientseite ein Objekt gesendet wird.  
...
Datenbanken sind eine weitere Mˆglichkeit um Daten speichern zu kˆnnen. Sie sind im Endeffekt eine Ansammlung von Tabellen, die mithilfe der SQL Abfragesprache genutzt werden kˆnnen.  
...
## Funktionen hˆherer Ordnung [Programmieren Lernen - Lerne alle Sprachen in einer Serie]
Hˆheren Funktionen ermˆglichen es einen anderen Typen aus einer typinferierten Funktion zur¸ckgeben, als man als Parameter ¸bergben hat. Man kann hier sogar FUnktionen zur¸ckgeben.  
(a->(b->c)->(b->c))  
...
Pattern Matching ist ein funktionaler Switch Case der es bei bspw. bei einer Liste ermˆglicht, gewisse Dinge auszuf¸hren, je nach dem wieviele Elemente sich innerhalb der Liste befinden.  
`liste = [1, 2, 3, 4, 5]`  
`switch (liste){`  
    `case[]:`  
        `print("liste leer");`  
...
Wildcards werden beim Pattern Matching dazu verwendet, gewisse Elemente ¸ber das Pattern auszulassen, die nicht mit behandelt werden.  
`liste = [1, 2, 3, 4, 5]`  
`switch (liste){`  
    `case[]:`  
        `print("liste leer");`  
...
Pattern Guards differenzieren die Abfragen noch mehr, in dem weitere Bedinungen hint dem Case definiert werden kˆnnen.
`liste = [1, 2, 3, 4, 5]`  
`switch (liste){`  
    `case[]:`  
        `print("liste leer");`  
    `case(x::[]):`  
        `print("erstes und einziges Element", x);`  
    `case(x::y::xs) y > 3:`  
        `print("erstes und alle Element ab dem dritten, wenn y grˆﬂer als 0 ist", x, y);`  
`}`
## Case Classes [Programmieren Lernen - Lerne alle Sprachen in einer Serie]
Case Classes kˆnnen im Rahmen des Pattern Matching, in dem man eine `case class` erstellt die die abzufragenden Inhalte enth‰lt.  
`case class MyClass{`  
    `int a;`  
    `int b;`  
    `String s;`  
...
Die Map-Funktion f¸hrt eine Funktion auf jedem Element einer Struktur aus.  
`l = [1,2,3,4,5,6]`  
`function foo(x){`  
    `return x + 2;`  
`}`  
...
Die Zip-Funktion dient zum zusammenf¸gen von mehreren Datenstrukturen zu einer. Das Ergebnis ist dann eine Liste aus Tupeln.  
`l = [1, 2, 3]`  
`l2 = [3, 4, 5]`  
...
Generatoren sind dazu gedacht unendlich lang ausgef¸hrt zu werden, benˆtigen aber nicht die notwendige Rechenleistung, da sie durch das `yield`-Schl¸sselwort angehalten wird.  
`function foo(){`  
    `a = 0;`  
    `b = 1;`  
    `yield a;`  
...
Hier wird die Funktion immer bis zum yield ausgef¸hrt und dann der Status der Funktion gemerkt.
## Dekoratoren [Programmieren Lernen - Lerne alle Sprachen in einer Serie]
Dekoratoren kˆnnen dazu genutzten werden um Funktionen durch weitere Funktionen zu erweitern.  
